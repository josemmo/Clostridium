...
def main(model, config, depth=None, wandbflag=False):
    ...
    with open(
        "/export/usuarios01/alexjorguer/Datos/HospitalProject/Clostridium/dev/" + config
    ) as file:
        config = yaml.load(file, Loader=yaml.FullLoader)

    main_path = config["main_path"]
    maldi_data_path = (
        main_path + "data/old/MALDI/data_processedMALDIQuant_noalign.pickle"
    )
    results = main_path + "results/"
