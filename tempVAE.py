    ...
    def __init__(
        ...
        for h_dim in hidden_dims:
            modules.append(
                nn.Sequential(
                    nn.Conv1d(
                        channels, out_channels=h_dim, kernel_size=3, padding="same"
                    ),
        ...
        self.decoder_input = nn.Sequential(
            ...
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.Linear(512, 2250 * 32),
            nn.BatchNorm1d(2250 * 32),
            nn.ReLU(),
        )
        hidden_dims.reverse()
        for i in range(len(hidden_dims) - 1):
            modules.append(
                nn.Sequential(
                    nn.ConvTranspose1d(
                        hidden_dims[i], hidden_dims[i + 1], kernel_size=3, padding=1
                    ),
        ...
        self.final_layer = nn.Sequential(
            nn.ConvTranspose1d(
                hidden_dims[-1], self.channels, kernel_size=3, dilation=1, padding=1
            ),
            nn.Upsample(scale_factor=2),
        )
